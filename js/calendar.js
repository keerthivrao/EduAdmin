 ...
 * the desired format.
 * @param Text-Input ctrl Must be a form control with a 'value' property. */
...
DATE_TIME_FIELD.prototype.show_calendar = function (initial_date_time)
{
  ...
  else
  {
	...
	if(this.as_string (dt)==this.as_string (new Date()))
	{
		this.control.value="";
	}
...
{
  return '<img src="' + this.expand_image_file_name (file_name) + '" title="' + title + '" alt="' + title + '">';
}
...
HTML_CALENDAR.prototype.draw_time_selector = function ()
{
  ...
  this.echo ('<div style="text-align: center"><input id="time' + this.id + '" class="' + this.text_control_style + '" type="text" value="' + format_date_time (this.today, 'H:i:s') + '" maxlength="8" size="8"></div>');
}
	...
	this.echo ('<tr><td class="' + this.banner_style + '">');
	  this.echo ('<table cellspacing="0">');
		this.echo ('<tr><td style="white-space: nowrap">');
			...
			this.echo (this.make_changer_link (this.previous_year, this.icon_as_html (this.previous_year_image, 'Previous year')));
		  this.echo (this.make_changer_link (this.previous_month, this.icon_as_html (this.previous_month_image, 'Previous month')));
		...
		this.echo ('</td><td style="white-space: nowrap">');
		  this.echo (this.make_changer_link (this.next_month, this.icon_as_html (this.next_month_image, 'Next month')));
		  if (this.show_year_selector)
			this.echo (this.make_changer_link (this.next_year, this.icon_as_html (this.next_year_image, 'Next year')));
		this.echo ('</td></tr>');
	  this.echo ('</table>');
	this.echo ('</td></tr>');
	this.echo ('<tr><td>');
...
WEBCORE_DATE_TIME_FIELD.prototype = new DATE_TIME_FIELD;
	...
	function configureDatePicker(control,title)
	{
		alert("1");
		initialfieldValue = control.value;
		alert("2");
		var testElement = document.getElementById(control);
		alert("3");
		field = new DATE_TIME_FIELD ();
		alert("4");
		field.attach(testElement,title);
		alert("5");
		field.show_time = false;
		alert("6");
		field.show_month_selector = true;
		alert("7");
		field.output_format = Date_format_iso;
		alert("8");					
		field.refresh();
		alert("9");
	...
	function showCalendar(control,title)
	{
		alert("9");
		configureDatePicker(control,title);
		alert("9");
		field.show_calendar();
